            ...
            child: Column(
              ...
              children: [
                ...
                CustomForm().buildTextField(
                  ...
                  padding: const EdgeInsets.symmetric(horizontal: 20),
                  validator: (value) {
                    if (CheckValidate().checkPW(focusNode: focusNodes[2], value: value!) == null && value.isNotEmpty) {
                ...
                Container(
                  ...
                  child: TextFormField(
                    ...
                    readOnly: true,
                    decoration: CustomForm().buildDefaultDecoration(labelText: data[7], isReadOnly: true),
                    onTap: () async {
                ...
                Container(
                  ...
                  child: TextFormField(
                    ...
                    decoration: CustomForm().buildDefaultDecoration(labelText: data[8]),
                    readOnly: controllers[7].text.isEmpty, // 첫 번째 필드가 비어있으면 readOnly 상태로 설정
                    validator: (value) {
                      return null;
                    },
                  ),
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      userInfo!.age = int.parse(controllers[3].text); // 문자열을 int로 변환
                      userInfo!.gender = controllers[4].text;
                      userInfo!.height = double.parse(controllers[5].text); // 문자열을 double로 변환
                      userInfo!.weight = double.parse(controllers[6].text); // 문자열을 double로 변환
                      if (userInfo!.roadAddress != controllers[7].text) {
                        userInfo!.NLatLng = [selectedadress!['x'], selectedadress!['y']];
                      }
                      userInfo!.roadAddress = controllers[7].text;
                      userInfo!.detailAddress = controllers[8].text;
                      String? result = await UserFirebase().updateUser(user: userInfo!);
                      if (result == null) {
                  ...
                  style: ElevatedButton.styleFrom(
                    foregroundColor: Colors.deepPurple,
                    backgroundColor: Colors.blueGrey[100],// 깔끔하면서도 세련된 색상 유지
                    minimumSize: Size(200, 45), // 크기를 조금 더 슬림하게 조정
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12), // 모서리 둥근 정도를 적당히 줄여 깔끔하게
                    ),
                    elevation: 0, //
